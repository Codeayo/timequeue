        ...
        <div class="story-grid">
          ...
          <div class="story-img-wrap">
            <img
              ...
              loading="lazy"
            />
            <div class="story-since-badge">
              <span class="since-label">Est.</span>
              <span class="since-year">2019</span>
            </div>
          </div>
          ...
          <div class="story-content">
            ...
            <p class="story-body">
              Our team sources seasonal ingredients from local farmers, collaborates with world-class sommeliers,
              and trains every member of our front-of-house to treat each guest like a personal invitation.
              This isn’t just dinner — it’s an evening you’ll carry with you.
            </p>

            <div class="story-stats" ref="statsRef">
              <div class="stat">
                <span class="stat-num">{{ counters[0].display }}{{ counters[0].suffix }}</span>
                <span class="stat-label">Years of Service</span>
              </div>
              <div class="stat-divider"></div>
              <div class="stat">
                <span class="stat-num">{{ counters[1].display }}{{ counters[1].suffix }}</span>
                <span class="stat-label">Seasonal Dishes</span>
              </div>
              <div class="stat-divider"></div>
              <div class="stat">
                <span class="stat-num">{{ counters[2].display }}{{ counters[2].suffix }}</span>
    ...
    <section class="testimonials-section reveal">
      <div class="container">
        <div class="testimonials-header text-center">
          <p class="testimonials-eyebrow">Guest Reviews</p>
          <h2>Words from Our Table</h2>
          <p class="text-muted">Real experiences from guests who've dined with us.</p>
        </div>

        <div class="testimonials-carousel">
          <transition name="testi-fade" mode="out-in">
            <div class="testi-card" :key="currentTestimonial">
              <div class="testi-stars">
                <span v-for="i in 5" :key="i" class="star">★</span>
              </div>
              <blockquote class="testi-quote">"{{ testimonials[currentTestimonial].quote }}"</blockquote>
              <div class="testi-author">
                <div class="testi-avatar">{{ testimonials[currentTestimonial].name.charAt(0) }}</div>
                <div>
                  <p class="testi-name">{{ testimonials[currentTestimonial].name }}</p>
                  <p class="testi-occasion">{{ testimonials[currentTestimonial].occasion }}</p>
                </div>
              </div>
            </div>
          </transition>
      ...
      <div class="container">
        ...
        <div class="features-grid">
          <div class="feature-card fade-up">
            ...
            <p>Book your preferred table in seconds. Receive instant confirmation and peace of mind.</p>
          </div>
          <div class="feature-card fade-up delay-100">
            <div class="icon-box">⏳</div>
            <h3>Intelligent Waitlist</h3>
            <p>When the house is full, join our smart waitlist. You'll automatically be promoted if a slot opens.</p>
          </div>
          <div class="feature-card fade-up delay-200">
            <div class="icon-box">🍽️</div>
            <h3>Full Itinerary View</h3>
            <p>Track all your upcoming reservations and waitlist statuses in one beautiful place.</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="cta-inner">
          ...
          <div class="cta-right">
            <div class="newsletter-box">
              <p class="newsletter-label">Stay in the Know</p>
              <h3 class="newsletter-title">Menu updates & exclusive offers, straight to your inbox.</h3>

              <transition name="nl-fade" mode="out-in">
                <div v-if="newsletterSuccess" key="success" class="newsletter-success">
                ...
                <form v-else key="form" @submit.prevent="handleNewsletter" class="newsletter-form">
                  ...
                  <p v-if="newsletterError" class="nl-error">{{ newsletterError }}</p>
                  <p class="nl-disclaimer">No spam, ever. Unsubscribe anytime.</p>
                </form>
              </transition>
            </div>
          </div>
...
const handleNewsletter = async () => {
  ...
  try {
    if (!newsletterEmail.value.includes('@')) throw new Error('Invalid email');
...
const counters = ref([
  { target: 6,  display: 0, suffix: '+' },
  { target: 40, display: 0, suffix: '+' },
  { target: 98, display: 0, suffix: '%' },
]);

const animateCounter = (counter, duration = 1800) => {
  ...
  const run = (now) => {
    ...
    counter.display = Math.round(eased * counter.target);
    ...
    (entries) => {
      if (entries[0].isIntersecting) {
        counters.value.forEach((c, i) => {
          setTimeout(() => animateCounter(c), i * 120);
        });
        observer.disconnect();
      }
    },
...
const testimonials = [
  ...
  {
    quote: 'I joined the waitlist on a whim and got promoted to a confirmed table within an hour. The platform is elegant, intuitive, and the food lived up to every expectation.',
    name: 'James T.',
    occasion: 'Business Dinner'
  },
  {
    quote: 'This is what fine dining reservations should always feel like. No phone calls, no hold music — just a few clicks and we had the perfect table for my mother\'s birthday.',
    name: 'Priya K.',
